...
makeCoreRow("non_routine_recovery_gt3_5", "Non-Routine Equipment Recovery >3.5 kn", 3, {
  windGt16: 1,
  windGt20: 3,
  windGt27: 9,
  waveGt1_5: 1,
  ...
  forecastStabilityLt6h: 2,
  reducedSpeedTowInterface: 3,
  closePassSpreadHazard: 4,
  electricalStorm: 9,
...
}, ["windGt27", "waveGt3_0", "visibilityLt1", "electricalStorm", "commsLost"]),
makeCoreRow("non_routine_recovery_lt3_5", "Non-Routine Equipment Recovery <3.5 kn", 2, {
  windGt16: 1,
  windGt20: 2,
  windGt27: 9,
  waveGt1_5: 1,
...
makeCoreRow("resupply_bunkering_crane_interface", "Resupply at Sea / Bunkering / Crane Interface", 2, {
  ...
  visibility1_2: 2,
  visibilityLt1: 9,
  recoveryBeforeDarknessNotAssured: 2,
  recoveryWindowNotAssured: 1,
  electricalStorm: 9,
  commsDegraded: 2,
  commsLost: 9,
  wbUnavailable: 1,
...
makeCoreRow("helicopter_operations", "Helicopter Operations", 2, {
  ...
  escortUnavailable: 1,
  frbStandbyUnavailable: 1,
}, ["windGt27", "visibilityLt1", "electricalStorm", "commsLost"], { escortUnavailable: true, frbStandbyUnavailable: true }),
makeCoreRow("emergency_drills", "Emergency Drills", 1, {
 ...
 makeCoreRow("offshore_gangway_transfers", "Offshore Gangway Transfers", 1, {
  ...
  visibility1_2: 3,
  visibilityLt1: 9,
  recoveryBeforeDarknessNotAssured: 9,
  recoveryWindowNotAssured: 3,
  forecastStabilityLt6h: 2,
  electricalStorm: 9,
  commsDegraded: 4,
...
if (tpOtcLogo) {
  ...
  const sectionTitle = (title: string, subtitle?: string) => {
    ensureSpace(50);
    setFillColor(soft);
    doc.roundedRect(left, y, width, subtitle ? 42 : 30, 10, 10, "F");
  ...
  const writeTextBlock = (text: string, fontSize = 9.5, color = navy, extraGap = 6) => {
    const lines = doc.splitTextToSize(text || "-", width - 4);
    ensureSpace(lines.length * 13 + extraGap);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(fontSize);
    setTextColor(color);
    doc.text(lines, left, y);
    y += lines.length * 13 + extraGap;
  };
 ...
 const drawRiskBanner = () => {
  ...
  const drawBulletList = (items: string[]) => {
    items.forEach((item) => {
      const lines = doc.splitTextToSize(item, width - 24);
...
const drawRowCard = (row: {
  ...
  sectionTitle("Assessment Details", "Primary header data for traceability, watch review and document control.");
  drawInfoGrid([
  ...
  sectionTitle("Active Condition Profile", "Only the highest active environmental band is applied in each exclusive family.");
  drawBulletList(
  ...
  sectionTitle("Row Evaluation", "Row-by-row scoring, emergency prompts, and hard-stop visibility.");
  ...
  if (notes) {
    sectionTitle("Operational Notes", "Bridge judgement, compensating controls, PTW references and operational remarks.");
    writeTextBlock(notes);
  }

  sectionTitle("Approval / Verification", "Manual sign-off block for controlled recordkeeping when required.");
  drawInfoGrid([
    ...
    <div className="flex items-start gap-4">
      ...
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
          RV Oruç Reis MOPO scoring engine with core vessel rows, WB phase logic,
          FRB emergency / exceptional-use logic, transfer rows, hard-stop catalogue,
          note codes, and numbered PDF assessment records.
        </p>
      </div>
    </div>

    <div className="grid w-full gap-3 sm:grid-cols-2 xl:grid-cols-3 lg:max-w-2xl">
      <div className="rounded-3xl border border-white/10 bg-white/5 px-4 py-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-2 text-sm font-bold leading-snug text-white break-words">
              {action.title}
            </div>
